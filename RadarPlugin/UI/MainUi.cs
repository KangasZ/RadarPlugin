...
public class MainUi : IDisposable
{
    ...
    private void DrawGeneralSettings()
    {
        ...
        if (ImGui.CollapsingHeader("Filtering Rules"))
        {
            ...
            {
                ImGui.SameLine();
                shouldSave |= UiHelpers.DrawCheckbox("Show Invisible Player Characters", ref configInterface.cfg.OverrideShowInvisiblePlayerCharacters, "Will show invisible player characters (like GMs).\nI have 0 proof that this works! If anyone can verify that'd be great.");
            }
